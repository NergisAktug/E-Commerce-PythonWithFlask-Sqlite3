...
def create_app():
    ...
    if not Kullanici.query.filter(Kullanici.email == 'member@example.com').first():
        kullanici = Kullanici(
    ...
    if not Kullanici.query.filter(Kullanici.email == 'admin@example.com').first():
        ...
        kullanici.rol.append(Roller(rolisim='Admin'))
        kullanici.rol.append(Roller(rolisim='Agent'))
        db.session.add(kullanici)
        db.session.commit()       
    db.create_all()
