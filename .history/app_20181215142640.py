...
def create_app():
    ...
    class alısveris:
        ...
        def __init__(self):
            self.sepetUrunismi=[]
            self.sepetUrunModeli=[]
            self.sepetUrunFiyati=[]
        def alısverisyap(self,urunismi,urunmodeli,urunfiyatı):
            self.sepetUrunismi.append(urunismi)
            self.sepetUrunModeli.append(urunmodeli)
            self.sepetUrunFiyati.append(urunmodeli)
        ...
        if request.method == 'GET':
            ...
            if data is not None:
                ...
                if Kullanici.query.filter_by(email=email, sifre=sifre, rolId=1).first():
                    ...
                    return render_template('index.html',rolId = 0, gir = session['uye_giris'])
            else:
                return render_template('uyeGiris.html')
    @app.route('/cikis')
    def cikis():
    ...
    def sepet(id):
        ...
        urunEkle=alısveris()
        urunEkle.alısverisyap(urun.urunismi,marka.marka_modeli,urun.urun.urunFiyati)
