...
def create_app():
    ...
    db.create_all()
    engine=create_engine('sqlite:///eticaret.sqlite')
    ...
    def guncelle(id):
        if request.method=='POST':
            try:
                ...
                markaId=request.form['markaId']
                db_uri='sqlite:///eticaret.sqlite'
                engine=create_engine(db_uri)
                conn=engine.connect()
                guncelleme=urunler.update().where(urunler.c.urun_id==id).values(urunismi=urunismi,urunresmi=urunresmi,fiyati=fiyati,markaId=markaId)
                conn.execute(guncelleme)
