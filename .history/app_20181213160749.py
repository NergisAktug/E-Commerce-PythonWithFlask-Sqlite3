        ...
        if request.method == 'GET':
            ...
            if data is not None:
                ...
                if Kullanici.query.filter_by(email=email, sifre=sifre, yetki=1).first():
                    ...
                    return render_template('admin.html',rolId = 1, giris = session['admin_giris'],urunler = urunler)
                else:
                    session['uye_giris'] = True
                    return render_template('index.html',rolId = 0, giris = session['uye_giris'],urunler = urunler)
            else:
                return render_template('uyeGiris.html')
    @app.route('/cikis')
    def cikis():
