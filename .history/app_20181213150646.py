...
def create_app():
    ...
    def kayit():
        if request.method == 'POST':
            ...
            yeniKullanici = Kullanici(email=mail, sifre=parola,yetki=0)
    ...
    def giris():
        if request.method == 'GET':
            ...
            if data is not None:
                session['girisyapildi'] = True
                if Kullanici.query.filter_by(email=email, sifre=sifre, yetki=1).first():
                    return render_template('index.html',yetki = 1, giris = session['girisyapildi'],urunler = urunler)
                else:
                    return render_template('index.html',yetki = 0, giris = session['girisyapildi'],urunler = urunler)
            else:
                return render_template('uyegirisi.html')
    @app.route('/cikis')
    def cikis():
