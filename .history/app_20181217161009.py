...
def create_app():
    ...
    class Sepet:
        ...
        def sepetgonder(self):
            return self.urunismi,self.urunFiyati,self.urunFiyati
    ...
    def sepetGoster():
        urunal=Sepet()
        dizi=urunal.sepetgonder()
        return render_template("sepet.html")
    @app.route('/sepet/<string:id>',methods=['POST','GET'])
    def sepet(id):
        urun=urunler.query.filter_by(urun_id=id).first()
        marka=markalar.query.filter_by(markaId=id)
        nesne=Sepet()
        for sutun2 in marka:
            nesne.sepeteUrunEkle(urun.urunismi,urun.urunFiyati,sutun2.marka_modeli)
