        ...
        if reguest.method=='POST':
            try:
                ...
                with sql.connect("eticaret.sqlite") as con:
                    cur=cor.cursor()
                    if urunler.query.filter_by(urunismi=urunismi).first() is not None:
                        cur=execute("UPDATE urunler SET urunresmi=?,fiyati=?,markaId=? WHERE urunismi=?",(urunresmi,fiyati,markaId,urunİsmi))
                        con.commit()
                        msg="güncelleme başarıyla yapıldı"
                except:
                    con.rollback()
