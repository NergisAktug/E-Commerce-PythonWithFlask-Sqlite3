...
def create_app():
    ...
    @app.route('/resim/<filename>') 
    def resim(filename): 
        urunresmi = request.files['images'] 
        if urunresmi: 
         filename = secure_filename(urunresmi.filename) 
         urunresmi_path = os.path.join(app.config['UPLOAD_FOLDER']+"urunresmi/", filename) 
         urunresmi.save(urunresmi_path) 
         urun.urunresmi = resim_path 
