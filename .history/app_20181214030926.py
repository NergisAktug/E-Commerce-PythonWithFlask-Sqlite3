...
def create_app():
    ...
    def anasayfa():
        ...
        return render_template('index.html',tumVeri=tumVeri,Urun=Urun)
    ...
    def guncelle(id):
        try:
            ...
            print(e)
        return redirect(url_for('urunGoster'))
    @app.route('/sepet',methods=['POST','GET'])
    def sepet():
        return render_template("sepet.html")
    @app.route('/urunCek/<string:id>')
    def urunCek(id):
        tumMarka=markalar.query.all()
        for marka in tumMarka:
            if marka.markaId==id:
                markaisim=marka.markaadi
                markamodel=marka.marka_modeli
                return render_template('index.html',markaisim=markaisim,markamodel=markamodel)
