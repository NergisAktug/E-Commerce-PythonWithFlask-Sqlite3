...
def create_app():
    ...
    def siparisgecmisi():
        if 'uye_giris' in session:
            ...
            gecmissiparis=siparis.query.filter_by(musteriId=kullaniciId).first()
            gecmissiparisleritut=[]
            for sipariss in gecmissiparis:
                urunn=urunler.query.filter_by(urun_id=sipariss.urunId).first()
                benimsiparisgecmisim={
                        'urunisim':str(urunn.urunismi)
                        'urunresmi':str(urunn.urunresmi)
                        'urunadet':sipariss.kactanesatinalınmıs
                        'satınalmatarihi':sipariss.siparisTarihi
